...
{
   /*printf("Test cnfg_file_name: %s/%sn%d  %d\n",cnfg_dir,nbase,last,my_rank);
   printf("Test cnfg_file_name size: %d %d %d\n",name_size("%s/%sn%d\0",cnfg_dir,nbase,last),NAME_SIZE,my_rank);
   printf("Test dummy cnfg_file_name size: %d %d %d\n",name_size("",cnfg_dir,nbase,last),NAME_SIZE,my_rank);*/
   /*BUG: name_size provides only on my_rank==0 correct values*/
   if(my_rank==0){
   if (noexp)
   ...
   for (nc=first;(iend==0)&&(nc<=last);nc+=step)
   {
      ...
      else
      {
         /*sprintf(cnfg_file,"%s/%sn%d",cnfg_dir,nbase,nc);*/
          sprintf(cnfg_file,"%s/conf.%04d",cnfg_dir,nc);
/*         random_ud();
         export_cnfg(cnfg_file);*/

         /*import_cnfg(cnfg_file);*/
         import_cnfg_tmlio(cnfg_file);
      }
